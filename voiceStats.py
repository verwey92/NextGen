...
voice_data = {}
# Structure: {channel_id: {user_id: { 'join_time': datetime, 'total_time': timedelta}}}
...
class VoiceStats(commands.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if before.channel is None and after.channel is not None:
            ...
            if user_id not in voice_data[channel_id]:
                voice_data[channel_id][user_id] = {
                    "join_time": now,
                    "total_time": timedelta(0),
                }
                print(
                    f"user joined channel {channel_id} @ {now} for total time of {timedelta(0)}"
                )
            # else:
        ...
        # User left a voice channel
            ...
            if (
                user_id in voice_data[channel_id]
                and "join_time" in voice_data[channel_id][user_id]
            ):
    ...
    async def voice_stats(self, ctx):
        ...
        for channel_id, users in voice_data.items():
            ...
            report.append(
                f"Channel {channel_name} - Total users: {total_users}, Average duration: {hh:02}:{mm:02}:{ss:02}"
            )
